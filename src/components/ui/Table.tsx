...
export function Table<T>({
  ...
  const sorted = (() => {
    ...
    return [...data].sort((a, b) => {
      const aValue = String(
        col.render ? col.render(a) : ((a as Record<string, unknown>)[sort.key] ?? ''),
      );
      const bValue = String(
        col.render ? col.render(b) : ((b as Record<string, unknown>)[sort.key] ?? ''),
      );
  ...
  const renderHeader = (col: Column<T>) => {
    ...
    return (
      <button
        ...
        className={cn(
          'flex items-center gap-1 font-medium focus:outline-none focus:ring-2 focus:ring-accent',
          active && 'text-accent',
        )}
